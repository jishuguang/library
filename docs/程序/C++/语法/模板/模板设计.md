...
```
- `std::iterator_traits` property traits template is provided to the kind of standard iterators
- `std::numeric_limits`

### Policy Traits

Some traits define how some types should be treated. We call them policy traits:

- `std::char_traits` is used as a policy traits parameter by the string and I/O stream classes
- `std::allocator_traits` added to be able to change the policy/behavior of allocators
