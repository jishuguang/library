...
论文基于流形学习分析CNN的假设为：神经网络的mainfold of interest能够嵌入到一个低维空间。

在上述假设的前提下，采用ReLU激活函数的CNN有如下特性：1)经ReLU后，若manifold of interest的体积不为空，那么这部分体积对应了一个线性变换；2)如果manifold of interest嵌入在输入空间的一个低维空间中，那么经ReLU后manifold of interest蕴含的信息能够得以完整保留。

- Linear Bottlenecks

基于流形学习对CNN的分析，论文提出Linear Bottleneck来捕获manifold of interest，同时避免nonlinear对bottleneck layer中蕴含信息的破坏（因为bottleneck layer维度较低，应用nonlinear会导致manifold of interest难以完全保留）。
...
[Squeeze and Excitation Networks](https://arxiv.org/abs/1709.01507)提出SE Block用于建模通道之间的关系、调整通道的输出，形式上类似于Attention机制。
